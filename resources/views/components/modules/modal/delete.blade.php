<button onclick="openDeleteModule({{ $index }})" class="w-7 h-7 rounded-full p-1.5 bg-indigo-500">
  <img src="/icons/programs/delete.svg" alt="" class="w-4 h-4">
</button>

<div id="deleteModule_{{ $index }}" class="fixed inset-0 bg-black/25 z-50 hidden mx-auto">
  ...
  <div class="flex items-center justify-center h-full">
    <div class="w-full max-w-lg bg-white rounded-xl absolute z-[9999]">
      <div class="w-full relative flex items-center py-5 indent-6 border-b">
        <p class="text-xl font-medium">Delete Module</p>
        <button onclick="closeDeleteModule({{ $index }})" class="absolute right-8 ">
          <img id="closeArchive" src="/icons/programs/close.svg" alt="" class="w-4 h-4cursor-pointer active:mt-1">
        </button>
      </div>
      {{-- Slot  --}}
      <div class="px-8 pt-6 pb-4">
        <form method ="POST" action="/modules/delete/{{$module->id}}" enctype="multipart/form-data" >
          @csrf
          @method('DELETE')
          <div class="space-y-8">
            <p class="text-sm"><span class="font-semibold">WARNING!</span> you are now deleting the module <span class="font-semibold">{{ $module->name }} </span>. Please keep in note that this will be deleted forever.</p>
            <div class="space-y-2">
...
<script>
function openDeleteModule(index){
  var deleteModule = document.getElementById('deleteModule_' + index);
  deleteModule.style.display = 'block';
}

function closeDeleteModule(index){
  var deleteModule = document.getElementById('deleteModule_' + index);
